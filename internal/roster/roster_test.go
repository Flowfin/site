...
// write, each tripping exactly one reason, and each reason naming the row so a
// reader opens the file at a line rather than bisecting it.
func TestEachRefusalIsTrippedByItsOwnFixture(t *testing.T) {
	for name, c := range map[string]struct{ file, says, names string }{
    ...
    "repository": "Flowfin/jellyfin-plugin-sso"`, 1),
		...
		if c.names != "" && !strings.Contains(got[0], c.names) {
			t.Errorf("%s was refused with %q, which does not name the row", name, got[0])
		}
	}
}
